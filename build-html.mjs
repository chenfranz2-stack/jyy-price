...
function calc() {
  ...
  const fabricCost = fabricPrice * usage;
  let addFees = 0;
  ...
  const total = fabricCost + p.baseLabor + addFees;
  ...
  document.getElementById('priceDetail').innerHTML =
    \`<b>\${t(p.key)}</b>（\${sz} · \${pt}）<br>
     \${t('fabric')}: \${money2(fabricPrice)}\${t('perMeter')} × \${usage.toFixed(2)}\${t('unitM')} = \${money2(fabricCost)}<br>
     \${t('labor')}: \${money2(p.baseLabor)}<br>
     \${t('fees')}: \${money2(addFees)}\${feeLines.length ? '（' + feeLines.join('，') + '）' : ''}<br>
